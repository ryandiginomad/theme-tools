...
export function checkJavascriptSourceStructure(source: string, context: Context): void {
  const error = findJavascriptSourceStructureError(source);
  if (!error) return;
...
function findJavascriptSourceStructureError(
  source: string,
): JavascriptSourceStructureError | undefined {
  let unclosed: JavascriptSourceStackEntry | undefined;
  ...
  for (const tag of liquidTagBodies(source)) {
    ...
    if (unclosed) {
      if (markup.tagName === 'endjavascript') {
        unclosed = undefined;
      }
      continue;
    }

    if (markup.tagName === 'javascript') {
    ...
    if (markup.tagName === 'endjavascript') {
      return { message: UNKNOWN_END_ERROR, startIndex: tag.start, endIndex: tag.end };
    }
  }
...
function hasJavascriptClosingTagAfter(source: string, startIndex: number): boolean {
...
interface JavascriptSourceStructureError {
  message: string;
  startIndex: number;
  endIndex: number;
}

interface JavascriptSourceStackEntry {
  startIndex: number;
  endIndex: number;
}
